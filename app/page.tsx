      ...
      <main className="bg-background">
        <section className="relative isolate flex min-h-[calc(100svh-4rem)] overflow-hidden bg-[#1a0824] text-white">
          <div className="pointer-events-none absolute inset-0 z-0">
            ...
            />
            <div className="absolute inset-0 bg-linear-to-r from-[#1a0824] via-[#24102f]/88 to-[#1a0824]/35" />
            ...
            <div className="max-w-3xl pb-8 sm:pb-12">
              <div className="animate-fade-up mb-5 inline-flex max-w-full items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3 py-2 text-xs backdrop-blur sm:px-4 sm:text-sm">
                ...
                <span className="truncate">
                  Premium hair rituals for a polished crown
                </span>
              </div>
              ...
              <p className="animate-fade-up-delay-2 mt-5 max-w-2xl text-base leading-7 text-violet-100 sm:mt-6 sm:text-lg sm:leading-8">
                High-performance hair essentials designed with clean textures,
                luminous shine, and salon-level polish.
              </p>
              <div className="animate-fade-up-delay-2 mt-8 flex flex-col gap-3 sm:mt-9 sm:flex-row">
                <Button
                  asChild
                  className="gold-glow h-12 rounded-full px-6 sm:px-7">
                  <Link href="/products">
                ...
                <Button
                  asChild
                  variant="outline"
                  className="h-12 rounded-full border-white/30 bg-white/10 px-6 text-white hover:bg-white hover:text-[#24102f] sm:px-7">
                  <Link href="/products?category=Hair%20Oil">
                    Explore hair oils
                  </Link>
                </Button>
              </div>
        ...
        <section className="mx-auto grid max-w-7xl gap-4 px-4 py-8 sm:grid-cols-3 sm:px-6 sm:py-10 lg:gap-5 lg:px-8">
          {[
            ...
            [
              "Texture first",
              "Made for curls, coils, waves, and silk press care.",
            ],
          ].map(([title, copy]) => (
            <div
              key={title}
              className="card-lift rounded-2xl border border-white/10 bg-white/96 p-5 shadow-sm shadow-black/10">
              <ShieldCheck className="mb-4 size-5 text-[#8b5a00]" />
              <h2 className="font-heading text-base font-semibold text-[#24102f]">
                {title}
              </h2>
        ...
        <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 sm:py-14 lg:px-8">
          <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h2 className="mt-3 text-3xl font-normal leading-[1.1] tracking-tight text-[#fff8df] sm:text-4xl">
            ...
            <Button
              asChild
              variant="outline"
              className="w-fit rounded-full hover-text-brand-muted">
        ...
        <section
          className="bg-[#fff8f0] px-4 py-14 sm:px-6 sm:py-16 lg:px-8"
          style={{
            background:
              "linear-gradient(135deg, #fff8f0 0%, #fdf0f8 50%, #f9f0ff 100%)",
          }}>
          <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-center lg:gap-12">
            <div className="relative isolate aspect-4/5 max-h-180 overflow-hidden rounded-[1.5rem] bg-violet-100 shadow-2xl shadow-[#1a0824]/20 sm:rounded-[2rem]">
              <Image
            ...
            <div className="max-w-2xl">
              ...
              <p className="mt-5 max-w-xl text-base leading-7 text-brand-muted sm:text-lg sm:leading-8">
                Every formula is designed around touch, shine, and consistency,
                so your routine feels considered from wash day to final detail.
              </p>
            </div>
          </div>
        </section>
