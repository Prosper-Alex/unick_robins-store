    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-3 md:hidden">
        {filteredUsers.length === 0 ? (
          <div className="rounded-xl border border-whibg-white/[0.08] px-4 py-8 text-center text-violet-100/55">
            No users found.
          </div>
        ) : (
          filteredUsers.map((user) => (
            <div
              key={user.id}
              className="rounded-xl border border-white/10 bg-white/8 p-4 text-white shadow-xl shadow-black/20 backdrop-blur">
              <p className="break-all font-medium" title={user.email}>
      ...
      <div className="hidden overflow-x-auto rounded-xl border border-white/10 bg-white/8 text-white shadow-xl shadow-black/20 backdrop-blur md:block">
        <Table className="min-w-180">
          <TableHeader>
            <TableRow className="border-white/10 bg-white/4 hover:bg-white/4">
              <TableHead className="font-semibold text-violet-100/70">
                Email
              </TableHead>
              <TableHead className="font-semibold text-violet-100/70">
                Role
              ...
              <TableHead className="text-right font-semibold text-violet-100/70">
                Orders
              </TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {filteredUsers.length === 0 ? (
              <TableRow className="border-white/10">
                <TableCell
                  colSpan={4}
                  className="h-24 text-center text-violet-100/55">
                  No users found.
                </TableCell>
              </TableRow>
            ) : (
              filteredUsers.map((user) => (
                <TableRow
                  ...
                  <TableCell className="font-medium" title={user.email}>
                    {maskEmail(user.email)}
                  </TableCell>
                  <TableCell>
                    <RoleSelect user={user} />
      ...
      <select
        ...
        className={`appearance-none rounded-full px-3 py-1 pr-8 text-xs font-medium capitalize outline-none transition-colors focus:ring-2 focus:ring-violet-500 disabled:opacity-50 ${
          role === "admin"
            ...
            : "bg-white/8 text-violet-100 ring-1 ring-white/10"
        }`}>
        <option value="customer">Customer</option>
        <option value="admin">Admin</option>
      </select>
      {loading && (
        <Loader2 className="absolute right-2 size-3 animate-spin text-stone-400" />
      )}
    </div>
  );
}
